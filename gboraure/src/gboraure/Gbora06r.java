...
@ManagedBean
@ViewScoped
public class Gbora06r extends Bd implements Serializable {
	...
	private LazyDataModel<Gbora06r> lazyModel;  
	
	
	/**
	 * @return the lazyModel
	 */
	public LazyDataModel<Gbora06r> getLazyModel() {
		return lazyModel;
	}	
...
public void init() {
	...
	lazyModel  = new LazyDataModel<Gbora06r>(){
		/**
		 * 
		 */
        ...
        @Override
		public List<Gbora06r> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String,Object> filters) { 
        	//Filtro
        	if (filters != null) {
	...
	private Date fecmue = new Date();
	private String rendimiento = "";
	private Object filterValue = "";
	private List<Gbora06r> list = new ArrayList<Gbora06r>();
	private List<Gbora06r> list1 = new ArrayList<Gbora06r>();
	private int validarOperacion = 0;
	private String zcodgra = "";
	private String zcodtab = "";
	private String zcodsec = "";
	private String zfecmue = "";
	...
	public Date getFecmue() {
		return fecmue;
	}

	public void setFecmue(Date fecmue) {
		this.fecmue = fecmue;
	}
	...
	public String getZfecmue() {
		return zfecmue;
	}

	public void setZfecmue(String zfecmue) {
		this.zfecmue = zfecmue;
	}
...
public void insert() throws  NamingException {   	
    ...
    try {
        ...
        String query = "INSERT INTO GBORA06R VALUES (1,'" + sdfecha2.format(fecmue) + "',?,?,?,?,'" + getFecha() + "',?,'" + getFecha() + "')";
        pstmt = con.prepareStatement(query);
        //pstmt.setString(1,vecgra[0].toUpperCase());
        pstmt.setString(1,vecsec[0].toUpperCase());
        pstmt.setString(2,vectab[0].toUpperCase());
        pstmt.setFloat(3, Float.parseFloat(rendimiento));
        ...
        try {
            ...
            if (e.getMessage().trim().equals("ORA-00001: unique constraint (OPENBIZVIEW.GBORA06R_PK) violated")){
            	
            	//System.out.println("se cumple la condicion y muestro el msg");
            	msj = new FacesMessage(FacesMessage.SEVERITY_FATAL, getMessage("gbora06rpkv"), "");
            }
...
public void delete() throws NamingException  {  
	...
	if (chkbox==null){
        ...
        try {
        	...
        	String query = "DELETE FROM GBORA06R WHERE CODGRA||TO_CHAR(FECMUE, 'DD/MM/YYYY')||CODSEC||CODTAB IN (" + param + ")";
            ...
            try {
                ...
                if (e.getMessage().trim().equals("ORA-02292: integrity constraint (OPENBIZVIEW.GBORA05_FK1) violated - child record found")){
                	...
                	//System.out.println("se cumple la condicion y muestro el msg");
                	msj = new FacesMessage(FacesMessage.SEVERITY_FATAL, getMessage("gbora06rfkv"), "");
                }
...
public void update() throws  NamingException {
	//System.out.println("entre al metodo UPDATE");
     ...
     try { 	 
        ...
        String query = "UPDATE GBORA06R";
         query += " SET RENDIMIENTO = ?, ";
         query += " USRACT = ?,";
         query += " FECACT = '" + getFecha() + "'";
         query += " WHERE CODGRA = 1 ";
         query += " AND CODSEC = ? ";
         query += " AND CODTAB = ? ";
         query += " AND TO_CHAR(FECMUE, 'DD/MM/YYYY') = '" + sdfecha2.format(fecmue) + "'";
         
         //System.out.println(query);
         
        pstmt = con.prepareStatement(query);

        pstmt.setFloat(1, Float.parseFloat(rendimiento));
        pstmt.setString(2, login);
        //pstmt.setString(3,vecgra[0].toUpperCase());
        pstmt.setString(3,vecsec[0].toUpperCase());
        pstmt.setString(4,vectab[0].toUpperCase());
 ...
 **/ 	
	public void select(int first, int pageSize, Object filterValue) throws SQLException, ClassNotFoundException, NamingException {
     ...
     String query = "SELECT * FROM"; 
		...
		query += "(SELECT A.CODGRA, TO_CHAR(A.FECMUE, 'DD/MM/YYYY') AS FECMUE, A.CODSEC, A.CODTAB, A.RENDIMIENTO, B.DESSEC, C.DESGRA, D.DESTAB ";
	    query += " FROM GBORA06R A, GBORA02 B, GBORA01 C, GBORA04 D";
	    query += " WHERE A.CODSEC = B.CODSEC";
	    query += " AND A.CODGRA = C.CODGRA";
	    query += " AND A.CODTAB = D.CODTAB";
	    query += " AND A.RENDIMIENTO||B.DESSEC||C.DESGRA||D.DESTAB||TO_CHAR(A.FECMUE, 'DD/MM/YYYY') LIKE '%" + ((String) filterValue).toUpperCase() + "%'";
    ...
    while (r.next()){
 	Gbora06r select = new Gbora06r();
 	select.setZcodgra(r.getString(1));
 	select.setZfecmue(r.getString(2));
 	select.setZcodsec(r.getString(3));
 	select.setZcodtab(r.getString(4));
 	select.setZrendimiento(r.getString(5));
 	select.setZdessec(r.getString(6));
 	select.setZdesgra(r.getString(7));
 	select.setZdestab(r.getString(8));
 	select.setZsector(r.getString(3) + " - " + r.getString(6));
 	select.setZgranja(r.getString(1) + " - " + r.getString(7));
 	select.setZtablon(r.getString(4) + " - " + r.getString(8));
    ...
    **/	
	...
	public void counter(Object filterValue) throws SQLException, ClassNotFoundException, NamingException {
			...
			//Consulta no paginada
		     String query = "SELECT A.CODGRA, TO_CHAR(A.FECMUE, 'DD/MM/YYYY') AS FECMUE, A.CODSEC, A.CODTAB, A.RENDIMIENTO, B.DESSEC, C.DESGRA, D.DESTAB ";
				    query += " FROM GBORA06R A, GBORA02 B, GBORA01 C, GBORA04 D";
				    query += " WHERE A.CODSEC = B.CODSEC";
				    query += " AND A.CODGRA = C.CODGRA";
				    query += " AND A.CODTAB = D.CODTAB";
